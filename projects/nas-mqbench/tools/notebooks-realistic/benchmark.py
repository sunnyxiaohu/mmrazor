...
EVALUATED_INDEXES = list(range(0, 32))  # len(api)))
# Create the API for realistic search space
nas_mqbench_pth = [
    'work_dirs/ptq_per-channel_w-minmax_a-minmax_bignas_resnet18_8xb256_in1k_calib32xb16_flops0-1000/search_epoch_5.pkl',
    'work_dirs/ptq_per-channel_w-minmax_a-minmax_bignas_resnet18_8xb256_in1k_calib32xb16_flops1000-2000/search_epoch_5.pkl',
    'work_dirs/ptq_per-channel_w-minmax_a-minmax_bignas_resnet18_8xb256_in1k_calib32xb16_flops2000-3000/search_epoch_5.pkl',
    'work_dirs/ptq_per-channel_w-minmax_a-minmax_bignas_resnet18_8xb256_in1k_calib32xb16_flops3000-4000/search_epoch_5.pkl',
]
...
######################GLOBAL SETTINGS#################################
nas_mqbench_pth2 = [
    'work_dirs/ptq_base_bignas_resnet18_8xb256_in1k_flops0-1000',
    'work_dirs/ptq_base_bignas_resnet18_8xb256_in1k_flops1000-2000',
    'work_dirs/ptq_base_bignas_resnet18_8xb256_in1k_flops2000-3000',
    'work_dirs/ptq_base_bignas_resnet18_8xb256_in1k_flops3000-4000',
]
...
for idx, dataset in enumerate(datasets):
      ...
      results = collections.OrderedDict(filter(lambda x: x[0] in metric_marker_mapping, results.items()))
      for jdx, metric in enumerate(results):
            xrange = np.array(results[base_metric]) - np.array(results[metric])
            max_idx, min_idx = np.argmax(xrange), np.argmin(xrange)
            print(f"Dataset {dataset}, Xrange between {base_metric} and {metric}: "
                  f"MDiff, Max, Min: {xrange.mean():.2f}, "
                  f"{xrange[max_idx]:.2f}[{results[base_metric][max_idx]:.2f} vs {results[metric][max_idx]:.2f}], "
                  f"{xrange[min_idx]:.2f}[{results[base_metric][min_idx]:.2f} vs {results[metric][min_idx]:.2f}]")
...
#       'retrained.per-tensor_w-minmax_a-minmax': ('^', 'yellow'),
#       'retrained.per-channel_w-minmax_a-minmax': ('v', 'purple'),
#       'retrained.mq_avg': ('*', 'yellow'),
#       'retrained.deployability_index': ('o', 'blue'),
#       'sliced.score': ('.', 'green'),
#       'sliced.per-tensor_w-minmax_a-minmax': ('^', 'yellow'),
#       'sliced.per-channel_w-minmax_a-minmax': ('v', 'purple'),
#       'sliced.mq_avg': ('*', 'purple'),
#       'sliced.deployability_index': ('o', 'blue')        
# }
# for idx, dataset in enumerate(datasets):
#       mq_args = ('per-tensor_w-minmax_a-minmax', 'per-channel_w-minmax_a-minmax')
...
#       plt.title('Kendall Rank Matrix')
#       plt.show()
#       fig.savefig(f'{WORK_DIR}/kendall_rank_hp_matrix_{dataset}.png')
#       fig, ax = plt.subplots()      
#       im = ax.imshow(s_corr, cmap='coolwarm', vmin=0.5, vmax=1)
#       for i in range(num_rows):
#             for j in range(num_cols):
...
#       plt.title('Spearmanr Rank Matrix')
#       plt.show()
#       fig.savefig(f'{WORK_DIR}/spearmanr_rank_hp_matrix_{dataset}.png')

#       sorted_fp32 = get_sorted_indices(results[f'retrained.{metric_on_set}'])
#       fig, ax = plt.subplots()
#       metric = 'deployability_index'
#       # if f'sliced.{metric}' in results or f'retrained.{metric}' not in results:
#       results.pop(f'sliced.{metric}', None)
#       results.pop(f'retrained.{metric}', None)
#       fig, ax = plt.subplots()
#       for jdx, metric in enumerate(results):
#             if metric not in metric_marker_mapping:
#                   continue
