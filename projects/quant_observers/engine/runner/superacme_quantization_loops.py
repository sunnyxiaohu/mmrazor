...
class SuperAcmeQATEpochBasedLoop(EpochBasedTrainLoop):
...
@LOOPS.register_module()
class SuperAcmeLSQEpochBasedLoop(SuperAcmeQATEpochBasedLoop):
...
@LOOPS.register_module()
class SuperAcmeQATValLoop(ValLoop):
...
@LOOPS.register_module()
class SuperAcmePTQLoop(TestLoop):
    ...
    def enable_hist(self):
        """Enable hist for this observer."""
        # from torch.ao.quantization import (FakeQuantizeBase)
        for _, mod in self.runner.model.named_modules():
            if isinstance(mod, torch.ao.quantization.FakeQuantizeBase):
                if hasattr(mod.activation_post_process, '_phase'):
                    mod.activation_post_process._phase = 'Collating Hist'
    ...
    def run(self) -> dict:
        ...
        if not self.only_val:
            ...
            if self.runner.model.quantizer.qconfig.a_fake_quant.p.keywords[
                    'observer'] in [
                        MODELS.get('KLObserver'),
                        MODELS.get('FasterMSEObserver')
                    ] or self.runner.model.quantizer.qconfig.w_fake_quant.p.keywords[
                        'observer'] in [
                            MODELS.get('KLObserver'),
                            MODELS.get('FasterMSEObserver')
                        ]:
                self.enable_hist()
                print_log('running calibratiion for phase2 Collating Hist!')
                for idx, data_batch in enumerate(self.dataloader):
                    if idx == self.calibrate_steps:
                        break
                    self.run_iter(idx, data_batch)
            ...
            print_log(
                'calibration cost time total is  {:.2f}s'.format(cost_time))
