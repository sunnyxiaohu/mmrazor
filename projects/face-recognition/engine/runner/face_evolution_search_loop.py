...
from mmrazor.engine.runner.utils import CalibrateBNMixin, check_subnet_resources, crossover


@LOOPS.register_module()
class FaceEvolutionSearchLoop(EpochBasedTrainLoop, CalibrateBNMixin):
    ...
    def __init__(self,
        ...
        if isinstance(calibrate_dataloader, dict):
            ...
            self.calibrate_dataloader = dataloader
    ...
    def run_epoch(self) -> None:
        ...
        self.candidates.sort_by(key_indicator=self.score_indicator, reverse=True)
        self.top_k_candidates = Candidates(self.candidates.data[:self.top_k])

        scores_after = self.top_k_candidates.resources(key_indicator=self.score_indicator)
    ...
    def sample_candidates(self) -> None:
        ...
        idx = 0
        print_feq = 50
        assert (self.num_candidates - len(self.candidates)) % self.runner.world_size == 0, \
        ...
        for rk in range(self.runner.world_size):
            self.candidates.extend(init_candidates_all[rk])

        assert len(self.candidates) == self.num_candidates
    ...
    def _save_searcher_ckpt(self) -> None:
        ...
        if self.runner.rank == 0:
            ...
            self.runner.logger.info(
                ...
                f'{self.top_k_candidates.resources(key_indicator=self.score_indicator)[0]}')
    ...
    def _init_predictor(self):
        ...
        if self.predictor.handler_ckpt:
            ...
            labels = np.array(self.candidates.resources(key_indicator=self.score_indicator))
