...
from mmrazor.models.algorithms.quantization.cle_superacme.common.connected_graph.connectedgraph import ConnectedGraph as AimetCommonConnectedGraph
from mmrazor.models.algorithms.quantization.cle_superacme.common.connected_graph.product import Product
from mmrazor.models.algorithms.quantization.cle_superacme.common.connected_graph.operation import Op, determine_preceding_op_input_product_index_in_multi_input_op
from mmrazor.models.algorithms.quantization.cle_superacme.model_module import PytorchModelModule
from mmrazor.models.algorithms.quantization.cle_superacme.meta.utils import is_leaf_module, run_hook_for_layers_with_given_input
from mmrazor.models.algorithms.quantization.cle_superacme import onnx_utils 
...
class ConnectedGraph(AimetCommonConnectedGraph):
    ...
    def get_op_type(model_cls: Type[torch.nn.Module]) -> str:
        ...
        if model_cls in onnx_utils.map_torch_types_to_onnx:
            ...
            print("unknown op_type -- defaulting to class name %s", op_type)
        return op_type
    ...
    def _determine_split_behavior_for_op_and_insert_split_op_in_connected_graph(self, op: Op):
        ...
        if len(output_product_names) > 1:
            ...
            if len(name_list) > 1:
                print("%s is a split Op", op.dotted_name)
    ...
    def _insert_split_op_in_connected_graph(self, preceding_op: Op, split_op: Op):
        ...
        for name in preceding_op_product_names:
            ...
            if preceding_op.name in name:
                ...
                print("Insert Split Op: Step 3. Deleted product: %s", deleted_product)
    ...
    def _add_consumers_to_split_op_product(self, preceding_op: Op, split_op_product: Product):
        ...
        for a_product_index in range(num_products):
            ...
            print("Insert Split Op: Step 2a. Consumer Op: %s, a_product_index: %s",
            ...
            print("Insert Split Op: Step 2b. Op has multiple input products: %s", a_consumer.inputs)
            ...
            print("Insert Split Op: Step 2c. For product: %s, split_op input_product_index: %s",
            ...
            for output in node_outputs:
                if output in connections_to_ir_nodes_dict:
                    if connections_to_ir_nodes_dict[output][0] is not None:
                        ...
                        print('Input of %s with name %s already exists. Ensure that no modules are being reused '
...
def _update_op_output_with_product(op: Op, product: Product):
    ...
    if op.output_shape is None:
        ...
        print('Mismatch between existing shape %s for product %s and output shape %s for '
...
def _get_module_tensor_shapes_entry(tensors: Union[torch.Tensor, List, Dict, None]):
    ...
    if isinstance(tensors, Dict):
        ...
        return shapes_dict
    print('Unexpected data type for tensor %s. Supported types include tensors, or Lists, Tuples, and Dicts of '
