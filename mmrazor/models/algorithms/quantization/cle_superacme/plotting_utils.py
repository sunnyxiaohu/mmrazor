...
from mmrazor.models.algorithms.quantization.cle_superacme.common.plotting_utils import style
from mmrazor.models.algorithms.quantization.cle_superacme.common import bokeh_plots
...
def get_activates(data):
    """
    Returns the activation of a conv_module in a 2d matrix, where each column is an output channel.

    :param data: activation
    :return: 2d numpy array
    """
    axis_0_length = data.shape[1]
    axis_1_length = np.prod(data.shape[2:])
    reshaped = data.reshape(int(axis_0_length), int(axis_1_length))
    activate = reshaped.T
    return activate
...
def line_plot_summary_statistics_model_activate(layer_name, layer_activates_data_frame, height, width):
    """
    Given a layer
    :param layer_name:
    :param layer_activates_data_frame:
    :return:
    """
    layer_activates = convert_pandas_data_frame_to_bokeh_column_data_source(layer_activates_data_frame)
    plot = figure(x_axis_label="Output Channels", y_axis_label="Summary Statistics",
                  title="activates Ranges per Output Channel: " + layer_name,
                  tools="pan, box_zoom, crosshair, reset, save",
                  width=width, height=height, output_backend="webgl")
    plot.line(x='index', y='min', line_width=2, line_color="#2171b5",
              legend="Minimum", source=layer_activates)
    plot.line(x='index', y='max', line_width=2, line_color="green",
              legend="Maximum", source=layer_activates)
    plot.line(x='index', y='mean', line_width=2, line_color="orange",
              legend="Average", source=layer_activates)
...
def visualize_activate_ranges_single_layer(activate, layer_name, scatter_plot=False):
    """
    Given a layer, visualizes activate ranges with scatter plots and line plots
    :param layer: layer with activates
    :param layer_name: layer name
    :param scatter_plot: Include scatter plot in plots
    :return: None
    """
    # device = get_device(activate)
    # activate.cpu()
    layer_activates = pd.DataFrame(get_activates(activate))
    layer_activates_summary_statistics = layer_activates.describe().T

    line_plots = line_plot_summary_statistics_model_activate(layer_name=layer_name,
                                                    layer_activates_data_frame=layer_activates_summary_statistics,
                                                    width=1000, height=700)

    if scatter_plot:

        scatter_plot_mean, scatter_plot_min = scatter_plot_summary_stats(layer_activates_summary_statistics,
                                                                         x_axis_label_mean="Mean activates Per Output Channel",
                                                                         y_axis_label_mean="Std Per Output Channel",
                                                                         title_mean="Mean vs Standard Deviation: " + layer_name,
                                                                         x_axis_label_min="Min activates Per Output Channel",
                                                                         y_axis_label_min="Max activates Per Output Channel",
                                                                         title_min="Minimum vs Maximum: " + layer_name)
